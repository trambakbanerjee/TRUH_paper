...
experiment.1<-function(n.u,n.v,d,pval.cutoff,reps,nB){
  ...
  set.seed(4)
  Sigma.4 = genPositiveDefMat("eigen",dim=d)$Sigma
  set.seed(5)
  mu4 = sample(c(4,-4),d,replace=TRUE)
  ...
  for(r in 1:reps){
    ...
    q<-runif(n.v,0,1)
    set.seed(50*r)
    V<-(q<=0.5)*mvrnorm(n.v,rep(0,d),Sigma.1)+(q>0.5)*mvrnorm(n.v,mu4,Sigma.4)
...
save.image(paste(getwd(),'/setting1_2.RData',sep=""))
