...
experiment.1<-function(n.u,n.v,d,pval.cutoff,reps,nB){
  ...
  for(r in 1:reps){
    ...
    q1<-runif(n.v,0,1)
    set.seed(50*r)
    Y2<-(q1<=0.5)*rmvgamma(n.v,shape=runif(d,5,5),rate=rep(0.5,d),Sigma.1)+(q1>0.5)*rmvexp(n.v,rate=rep(1,d),Sigma.2)#rmvexp(n.v,rate=rep(0.5,d),Sigma.2)
    set.seed(30*r)
...
save.image(paste(getwd(),'/setting3_2.RData',sep=""))
