...
experiment.1<-function(n.u,n.v,d,pval.cutoff,reps,nB){
  ...
  set.seed(1)
  express.threshold = c(runif(0.8*d,0.5,0.6),rep(0,0.2*d))
  
  thresh.u<- matrix(rep(express.threshold,n.u),n.u,byrow = T)
  thresh.v<- matrix(rep(express.threshold,n.v),n.v,byrow = T)
  ...
  for(r in 1:reps){
    ...
    U<- (p2>thresh.u)*Y1
    set.seed(50*r)
    Y2<-rmvexp(n.v,rate=rep(1,d),Sigma.2)
    set.seed(30*r)
...
save.image(paste(getwd(),'/setting3_1.RData',sep=""))
