...
if(Others){
  ...
  save(list=c('test1.energy','test1.crossmatch','out.gtests'),file="case2_don2_others.RData")
  
}

#----------------- Proposed ---------------------------
if(TRUH){
  out<-truh(V.sub,UN.sub,B=500,fc=1.1,a=pval.cutoff)#truh(V.sub,UN.sub,B=500,fc=1.15,a=pval.cutoff)
  
  save('out',file="case2_don2_truh_new.RData")
}
